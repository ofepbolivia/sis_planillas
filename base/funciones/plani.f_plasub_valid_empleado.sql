...
BEGIN
	...
	v_existe = 'no';
	select id_tipo_planilla, per.id_periodo, fecha_ini, fecha_fin, id_uo, p.id_usuario_reg, p.momento_planilla
    into v_planilla
          ...
          where uofun.id_funcionario = ' || p_id_funcionario || ' and tc.codigo in (''PLA'', ''EVE'') and UOFUN.tipo = ''oficial'' and
              ...
              uofun.estado_reg != ''inactivo'' and uofun.id_funcionario not in (
                  ...
                  where 	fp.id_funcionario = uofun.id_funcionario and
                          ...
                          p.id_periodo = ' || v_planilla.id_periodo || ' and coalesce(p.momento_planilla,''normal'') = '''||v_planilla.momento_planilla||''')
...
ALTER FUNCTION plani.f_plasub_valid_empleado (p_id_usuario integer, p_id_funcionario integer, p_id_planilla integer, p_forzar_cheque varchar, p_finiquito varchar, p_contrato varchar, out o_id_funcionario_planilla integer, out o_tipo_contrato varchar)
  OWNER TO postgres;