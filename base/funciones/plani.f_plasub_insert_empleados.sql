...
BEGIN
    ...
    v_filtro_uo = '';
	select id_tipo_planilla, per.id_periodo, per.fecha_ini, per.fecha_fin, id_uo, p.id_usuario_reg, p.id_gestion, p.momento_planilla
    into v_planilla
    from plani.tplanilla p
    inner join param.tperiodo per on p.id_periodo = per.id_periodo
    ...
    --raise 'a: %,fecha_ini: %, b: %, c: %, d: %, e: %, f: %, g: %',v_filtro_uo,v_planilla.fecha_ini,v_planilla.fecha_fin, v_fecha_ini, v_planilla.id_tipo_planilla,  v_planilla.id_periodo, v_planilla.id_gestion,v_planilla.momento_planilla;
    for v_registros in execute('
          ...
          where tc.codigo in (''PLA'', ''EVE'') and UOFUN.tipo = ''oficial'' and '
              ...
              uofun.estado_reg != ''inactivo'' and uofun.id_funcionario not in (
                  ...
                  where 	fp.id_funcionario = uofun.id_funcionario and
                          ...
                          p.id_periodo = ' || v_planilla.id_periodo || ' and p.id_gestion = '||v_planilla.id_gestion||' and coalesce(p.momento_planilla,''normal'') = '''||v_planilla.momento_planilla||''')
    	...
    	if v_planilla.momento_planilla = 'extraordinaria' then
            if (select case when td.id_funcionario is not null then true else false end
                from plani.tdescuento_bono td
                where td.id_funcionario = v_registros.id_funcionario and (v_planilla.fecha_ini  between td.fecha_ini and td.fecha_fin) and (v_planilla.fecha_fin between td.fecha_ini and td.fecha_fin) and td.tipo_desc_bono = 'extraordinaria' limit 1) then
        ...
        elsif v_planilla.momento_planilla = 'normal' then
            if (exists(
                  select 1
                  ...
                  tc.codigo in ('SUBLAC','SUBPRE','SUBSEP') and db.fecha_ini <= v_planilla.fecha_ini and
...
ALTER FUNCTION plani.f_plasub_insert_empleados (p_id_planilla integer)
  OWNER TO postgres;